...
2. **対局モード** を選ぶ。
   ...
   - **エージェント対エージェント**: **黒を担当するエージェント** と **白を担当するエージェント** をそれぞれ選ぶ（同じエージェントでもよい）。開始後は終局まで自動で進む。
...
カタログには「生成 AI (呼称)」のように表示されます。呼称はカタログ内で重複してはいけません。ファイルを書いたあと、カタログ画面を開き直してください。一覧に現れないときはサービスを起動し直してください。
