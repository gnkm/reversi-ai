...
│   │   ├── rl-stage3.json             # 段階 3。パターン TD(λ) と段階 2・位置評価の比較
│   │   ├── rl_stage3.py               # 段階 3。パターン評価を同じ深さの αβ で比べる。CI では走らせない
│   │   ├── rl-stage3.md               # 段階 3 の知見。人手。JSON から生成しない
│   │   ├── rl-stage4.json             # 段階 4。同じパターン評価で完全読みの有無を比べる
│   │   ├── rl_stage4.py               # 段階 4。完全読みあり／なしを同じ開始局面で比べる。CI では走らせない
│   │   ├── rl-stage4.md               # 段階 4 の知見。人手。JSON から生成しない
...
│   │   │   ├── rl_pattern.py          # 強化学習 (パターンの読み)。パターン評価を葉にした深さ 4 αβ。空きマスが少なければ終局まで読む
...
| `rl_pattern.py` | 強化学習 (パターンの読み) | `models/rl-pattern.json` のパターン評価を葉にした深さ 4 の αβ。空きマスが閾値以下ならヒューリスティックを使わず終局まで完全読みし、葉は最終石数差（自分 − 相手）である。閾値は 1 手の思考時間の上限から決める（shall ではない）。学習は自己対局の TD(λ) で、WTHOR を使わない。既存の線形 RL は置き換えない。既存「強化学習 (自己対局)」「強化学習 (自己対局＋読み)」「強化学習 (対称な読み)」は終盤完全読みを持たないまま残す。NN 推論も OpenRouter も使わない |
...
| 版 | 日付 | 内容 |
| --- | --- | --- |
| 0.1.47 | 2026-09-22 | 強化学習 (パターンの読み) が空きマス閾値以下で終盤を完全読みする |
| 0.1.46 | 2026-09-22 | 段階 3 の比較 JSON と知見を置く |
