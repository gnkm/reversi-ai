...
| `jev.py` | 生成 AI (Jev) | `typesafe/jev-1.13` の Decisions API。1 着手 1 呼出しで、合法手を選択肢とする Choice を送る。盤面配列は渡さない。マスの種類・角・角渡し・相手手数・裏返しはコードが言葉にして渡す。`jev.py` が Choice の確率・confidence と着手後 1 手評価を合成する。合成時、各合法手のコード評価・指標・確率・confidence・最終選択を標準エラーへ 1 行ずつ出す。指示は `prompts/jev.json`。合法手の外を採用しない |
...
| 版 | 日付 | 内容 |
| --- | --- | --- |
| 0.1.22 | 2026-09-20 | ルールベース (αβ) をカタログに載せ、深さ 6 の Negamax で着手する |
