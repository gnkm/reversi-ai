...
│   │   │   ├── alphabeta.py           # ルールベース (αβ) 深さ 6。Move Ordering。Transposition Table（Zobrist ハッシュ）。葉は Mobility・Corner・X/C・Frontier・石差。W は空きマス（Game Phase）
...
| 版 | 日付 | 内容 |
| --- | --- | --- |
| 0.1.32 | 2026-09-20 | ルールベース (αβ) の探索に Transposition Table（Zobrist ハッシュ）を入れる |
